...
// Construit un fragment qui accepte les flexions françaises courantes d'un
// radical : masculin, féminin, pluriels. Écrire `flechi("annul")` plutôt que
// `annulee?` évite l'angle mort qui a coûté plusieurs tours de correction : une
// forme oubliée ne déclenche rien, et rien ne le signale.
// Matcher: radical + optionnel e + optionnel e + optionnel s
// Exemples avec "annul":
// - annul (pas d'accord)
// - annule (masc sing)
// - annulee (fem sing)
...
const REGLES: readonly Regle[] = [
  ...
  { nom: "engagement.annulation", categorie: RiskCategory.ENGAGEMENT, motif: /\b(annul(?:e(?:e)?)?(?:s)?|annuler|decal(?:e(?:e)?)?(?:s)?|reporter|je peux plus)\b/ },
  ...
  { nom: "fait.relation", categorie: RiskCategory.FACT, motif: /\btu es (celibataire|en couple|mari(?:e(?:e)?)?(?:s)?)\b|\btu as (une copine|quelqu'?un|des enfants)\b/ },
  ...
  { nom: "emotionnel.conflit", categorie: RiskCategory.EMOTIONAL, motif: /\b(decu(?:e(?:e)?)?(?:s)?|blesse(?:e(?:e)?)?(?:s)?|vexe(?:e(?:e)?)?(?:s)?|en colere|tu m'?ignores|tu reponds jamais|ca me fait mal)\b/ },
  { nom: "emotionnel.detresse", categorie: RiskCategory.EMOTIONAL, motif: /\b(je vais mal|deprime(?:e(?:e)?)?(?:s)?|j'?en peux plus|je suis triste|aide moi)\b/ },
  ...
  { nom: "argent.vocabulaire", categorie: RiskCategory.MONEY, motif: /\b(?:un|une|le|la|les|des|ce|ces|mon|ton|son|notre|votre|leur|mes|tes|ses|nos|vos|leurs)\s+prets?\b|\bpret\s+(?:de|bancaire)|\b(credits?|dettes?|rembours(?:e(?:e)?)?(?:s)?|rembourser|virements?|mobile money|wave|orange money)\b|\bbesoin d'?argent\b/ },
  ...
  { nom: "intime.explicite", categorie: RiskCategory.INTIMATE, motif: /\b(nu(?:e(?:e)?)?(?:s)?|nudes?|sexe|coucher ensemble|au lit avec)\b/ },
