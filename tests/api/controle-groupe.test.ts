...
describe("réglage du groupe de contrôle", () => {
  beforeEach(() => {
    ...
    listGroups.mockResolvedValue([POSTE, COMMUNAUTE, AMIS]);
  ...
  it("écarte les communautés de la liste proposée", async () => {
    ...
    expect(corps.groupes.map((g) => g.jid)).toEqual([AMIS.jid, POSTE.jid]);
  });
  ...
  it("enregistre le groupe choisi et poste un message de vérification", async () => {
    ...
    await expect(reponse.json()).resolves.toMatchObject({ confirmationEnvoyee: true });
  });
  ...
  it("garde le réglage quand le message de vérification ne part pas", async () => {
    // Perdre le choix parce qu'un envoi a échoué obligerait à tout recommencer,
    // alors que la panne peut être passagère.
    sendText.mockRejectedValue(new Error("GOWA injoignable"));
    const { PUT } = await import("@/app/api/controle/groupe/route");
    const reponse = await PUT(choix(POSTE.jid));
    expect(reponse.status).toBe(200);
    expect(enregistrerGroupeDeControle).toHaveBeenCalledOnce();
    await expect(reponse.json()).resolves.toMatchObject({ confirmationEnvoyee: false });
    expect(lignesJournal.join("\n")).toMatch(/confirmation non postée|Message de confirmation non posté/i);
  });
